...
public class QMatrixGenerator extends DECAF {
	...
	public double[][] generateQMatrix() {
		...
		// This takes advantage of multi-threading to speed up calculations. We
		// could multi-thread the calculation of
		// every cell, however to lower overhead and increase performance, we
		// will only multi-thread the row
		 ...
		 * 
		 * while( tfd.hasNext() ){ int[] next = tfd.next();
		 * 
		 * if( next == null ) continue;
		 * 
		 * // Run TreeGenerator //tg.getFailureRate(transitionStates, next[0],
		 * next[1]); }
		 * 
		 * tg.buildTree(transitionStates);
		 * 
		 * // Add up diagonals qMatrix = threadPool.invoke(new
		 * QMatrixSumDiagonalsAction(qMatrix));
		 */
		return qMatrix;
	}
